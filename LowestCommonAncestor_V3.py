...
Output.write("#Taxon name,Taxid,Reads mapping to taxid,Reads mapping to children taxids,Pearson skewness score\n")
...
for k,v in AllTaxidDict.iteritems():
	...
	if len(descendants) == 1: #This is a leaf node
		...
		Output.write(str(name.itervalues().next()) + "," + str(k) + "," + str(v) + ",0," + str(PearsonSkewness) + "\n")
	else:
		...
		if PearsonSkewness == "":
			...
			Output.write(str(name.itervalues().next()) + "," + str(k) + "," + str(v) + "," + str(ChildrenCount) + "," + str(PearsonSkewness) + "\n")
Output.close()
